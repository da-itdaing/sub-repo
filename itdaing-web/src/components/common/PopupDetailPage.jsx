...
export function PopupDetailPage({
  ...
  const { reviews, loading: reviewsLoading, error: reviewsError, average, refetch: refetchReviews } = useReviewsByPopupId(popupId);
  ...
  // popup, seller가 없을 때도 안전하게 처리
  const operatingHoursLabel = useMemo(() => {
    if (!popup?.operatingHours) return "운영 시간 정보 없음";
    return popup.operatingHours.map(item => `${item.day} ${item.time}`).join(" / ");
  }, [popup?.operatingHours]);
  ...
  const popupData = useMemo(() => {
    ...
    return {
      ...
      imageUrl: getImageUrl(popup.thumbnail, "https://images.unsplash.com/photo-1555099962-4199c345e5dd?w=800&h=600&fit=crop"),
      sellerName: seller.name,
      sellerImage: getImageUrl(seller.profileImage, defaultSellerImage),
      content: {
        title: seller.category ?? seller.name,
        description: popup.description,
      },
    };
  }, [popup, seller, likesCount, operatingHoursLabel, defaultSellerImage]);
  ...
  if (!popupData) {
    return <div className="flex items-center justify-center min-h-screen">데이터를 불러오는 중입니다...</div>;
  }
      ...
      <div className="max-w-[1344px] mx-auto overflow-clip px-3 sm:px-4 lg:px-0">
        ...
        <div className="max-w-[930px] mx-auto">
          ...
          <div className="relative mb-3 sm:mb-4 lg:mb-[18px]">
            <div className="h-56 sm:h-80 lg:h-[280px] xl:h-[320px] rounded-[10px] lg:rounded-[5px] overflow-hidden mx-auto max-w-full lg:max-w-[450px] xl:max-w-[500px] shadow-sm bg-gray-100 flex items-center justify-center">
              <img
                alt={popupData.title}
              ...
              <div className="mb-8 lg:mb-[40px] flex flex-col lg:flex-row justify-between items-start gap-6 lg:gap-8">
                ...
                <div className="flex-1 w-full">
                  ...
                  <div className="flex items-start gap-2 mb-2 lg:mb-[10px]">
                    <MapPin className="size-5 lg:size-[22px] shrink-0 mt-1" />
                ...
                <button
                  ...
                  className="flex flex-col items-center shrink-0 hover:opacity-80 transition-opacity cursor-pointer"
                >
          ...
          {activeTab === "지도" && (
            <>
              ...
              <div className="mb-3 lg:mb-[14px] flex items-start lg:items-center gap-2">
                <MapPin className="size-5 lg:size-[23px] shrink-0 mt-1 lg:mt-0" />
                ...
                <MapPin className="size-5 lg:size-[22px] shrink-0 mt-1 lg:mt-0 hidden sm:block" />
              </div>

              {/* 실제 지도 */}
              <div className="w-full lg:w-[911px] mx-auto mb-8 lg:mb-[60px]">
                {popup?.latitude != null && popup?.longitude != null ? (
                  <KakaoMap
                    ...
                    height="h-64 sm:h-80 lg:h-[455px]"
                    level={3}
                    showControls={true}
                  />
                ) : (
                  <div className="h-64 sm:h-80 lg:h-[455px] bg-gray-100 rounded-2xl flex items-center justify-center text-gray-500">
                  ...
                  <div className="w-full lg:w-1/2 space-y-2 lg:space-y-3">
                    {ratingDistribution.map((item, index) => (
                      <div key={index} className="flex flex-row items-center gap-2 lg:gap-4">
                        <span className="font-['Inter:Regular','Noto_Sans_KR:Regular',sans-serif] text-sm lg:text-xl text-black w-10 lg:w-[50px] shrink-0">
                          {item.label}
                        </span>
              ...
              {sortedReviews.map((review, index) => (
                ...
                >
                  <div className="flex gap-3 lg:gap-4 mb-4">
                    <img
                      alt=""
                      className="size-16 lg:size-[120px] rounded-full object-cover shrink-0"
