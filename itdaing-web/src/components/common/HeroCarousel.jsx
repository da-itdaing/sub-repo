...
export function HeroCarousel({ items, onPopupClick, isLoggedIn, onLoginClick }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(
      ...
      config.isMobile ? 3000 : 4000
    );
    return () => clearInterval(interval);
  }, [isPaused, config.isMobile, carouselItems.length]);
  ...
  return (
    <div className="mt-4 sm:mt-6 md:mt-8 mb-0">
      <div
        ref={containerRef}
        className="relative rounded-lg bg-white md:bg-transparent overflow-hidden md:overflow-x-visible md:overflow-y-hidden"
        onMouseEnter={() => setIsPaused(true)}
        onMouseLeave={() => setIsPaused(false)}
      >
        {/* Mobile version */}
        <div
          className="block md:hidden py-8"
          style={{ minHeight: Math.max(300, Math.round(config.mobile.cardH + 80)) }}
        >
          <div
            className="relative mx-auto px-4"
            style={{ height: config.mobile.trackH, maxWidth: "100%", position: "relative" }}
          >
            {/* Drag layer */}
            <motion.div
                      ...
                      <div className="relative w-full h-full">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-2 text-white">
                          {item.rank && position === 0 && (
                            <div className="mb-1">
                              <span className="text-lg font-['Black_Han_Sans:Regular',sans-serif]">
                                - {item.rank} -
                              </span>
                            </div>
                          )}
                          <h3
                            className={`font-['Black_Han_Sans:Regular',sans-serif] mb-1 line-clamp-2 ${
                              position === 0 ? "text-[13px]" : "text-[9px]"
                            }`}
                          >
                            {item.title}
                          </h3>
                          {position === 0 && (
                            <>
                              <p className="text-[9px] font-['Black_Han_Sans:Regular',sans-serif] mb-0.5">
                                {item.date}
                              </p>
            ...
            {carouselItems.map((_, idx) => (
              <button
                ...
                className={`transition-all rounded-full ${
                  currentIndex === idx ? "w-6 h-2 bg-[#eb0000]" : "w-2 h-2 bg-gray-400"
                }`}
        ...
        {/* Desktop version */}
        <div
          className="hidden md:block py-14"
          style={{ minHeight: Math.max(520, config.desktop.cardH + 160) }}
        >
          <div
            className="w-full mx-auto px-4 lg:px-16"
            style={{ maxWidth: config.desktop.containerMaxW, height: config.desktop.trackH, position: "relative" }}
          >
            {/* Slides */}
            <div className="absolute inset-0 flex items-center justify-center">
              {carouselItems.map((item, idx) => {
                ...
                const scaleMap = config.desktop.scaleMap;
                const opacityMap = config.desktop.opacityMap;
                ...
                return (
                  <motion.div
                    ...
                    className={`absolute rounded-2xl overflow-hidden shadow-xl border cursor-pointer bg-black/10 backdrop-blur-sm ${
                      pos === 0 ? "ring-2 ring-[#eb0000]" : "ring-0"
                    }`}
                    ...
                    <div className="relative w-full h-full">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-sm font-semibold tracking-wide bg-[#eb0000] px-2 py-0.5 rounded-full shadow">
                            {item.rank}위
                          </span>
                        </div>
                        <h3 className="font-['Black_Han_Sans:Regular',sans-serif] text-lg leading-tight line-clamp-2 mb-2 drop-shadow">
                          {item.title}
                        </h3>
                        {pos === 0 && (
                          <>
                            <p className="text-xs mb-1 opacity-90 font-['Pretendard:Regular',sans-serif]">
                              {item.date}
                            </p>
                            <p className="flex items-center gap-1 text-xs opacity-90 font-['Pretendard:Regular',sans-serif]">
...
function useResponsiveCarouselConfig() {
  const calculate = () => {
    ...
    // Desktop presets define responsive scaling factors rather than fixed pixels
    const presets = [
      { breakpoint: 1536, widthFactor: 0.20, gapFactor: 0.78, maxDistance: 3 },
      { breakpoint: 1440, widthFactor: 0.22, gapFactor: 0.77, maxDistance: 3 },
      { breakpoint: 1280, widthFactor: 0.24, gapFactor: 0.76, maxDistance: 2 },
      { breakpoint: 1024, widthFactor: 0.26, gapFactor: 0.74, maxDistance: 2 },
      { breakpoint: 900, widthFactor: 0.30, gapFactor: 0.72, maxDistance: 2 },
      { breakpoint: 768, widthFactor: 0.34, gapFactor: 0.70, maxDistance: 1 },
    ];
    ...
    const mobile = {
      ...
      step,
      scale: { 0: 1, 1: 0.89, 2: 0.81, 3: 0.75 },
      opacity: { 0: 1, 1: 0.7, 2: 0.5, 3: 0.3 },
    };

    const desktopPreset = presets.find(p => w >= p.breakpoint) ?? presets[presets.length - 1];
    const baseCardW = Math.round(
      Math.min(
        Math.max(w * desktopPreset.widthFactor, 200),
        Math.min(380, w * 0.36)
      )
    );
    const cardH = Math.round(baseCardW * 1.45);
    const containerMaxW = Math.min(w * 0.92, 1680);
    const desktop = {
      cardW: baseCardW,
      cardH,
      step: Math.round(baseCardW * desktopPreset.gapFactor),
      maxDistance: desktopPreset.maxDistance,
      containerMaxW,
      trackH: cardH + 40,
      scaleMap: { 0: 1, 1: 0.88, 2: 0.78, 3: 0.7 },
      opacityMap: { 0: 1, 1: 0.85, 2: 0.55, 3: 0.4 },
    };
