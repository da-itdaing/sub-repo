...
import { useAllReviews } from "../../hooks/usePopups";

interface MyPageReviewsProps {
	reviewPopupIds?: number[];
}

export function MyPageReviews({ reviewPopupIds }: MyPageReviewsProps){
	const { reviews, loading } = useAllReviews(reviewPopupIds);
	if (loading) {
		...
		<div className="p-4 space-y-4">
			{reviews.map(r => (
				<div key={r.id} className="border rounded p-4 bg-white">
					<p className="font-semibold mb-1">{r.content.slice(0, 30)}...</p>
