...
interface MyPageProps {
	...
	onRecommendClick?: (id: number) => void;
}

type TabKey = "overview" | "recommend" | "favorites" | "reviews" | "schedule";
...
export function MyPage({ onClose, onPopupClick, onRecommendClick }: MyPageProps){
	const { user, logout } = useAuth();
	const { profile, loadPresetFor } = useUser();
	const { data: popupList } = usePopups();
	const [activeTab, setActiveTab] = useState<TabKey>("recommend");

	useEffect(() => {
		if (!profile) {
			loadPresetFor("consumer1");
		}
	}, [profile, loadPresetFor]);
	...
	return (
		<div className="min-h-screen bg-[#f8f8f8] pt-16 pb-20">
			...
			<main className="max-w-[1080px] mx-auto px-4 py-6 space-y-8">
					...
					<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
						...
						<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
							<div className="p-3 rounded-xl bg-[#f9fafb]">
								...
								<p className="text-lg font-semibold text-gray-900">{favoriteIds.length}</p>
							</div>
							<div className="p-3 rounded-xl bg-[#f9fafb]">
							...
							{recommendations.map(popup => (
								...
								>
									<div className="h-44 overflow-hidden">
										<img src={popup.thumbnail} alt={popup.title} className="w-full h-full object-cover" />
									</div>
				...
				{activeTab === "reviews" && (
					<section className="bg-white rounded-2xl border border-gray-100 shadow-sm">
						<MyPageReviews reviewPopupIds={favoriteIds} />
					</section>
				)}
