...
const sanitizeMarkdown = (text) => {
  ...
  // 10. **bold** 텍스트 앞에 줄바꿈 추가 (한글/숫자 뒤)
  result = result.replace(/([가-힣0-9])(\*\*[^*]+\*\*)/g, '$1\n\n$2');
  
  // 11. 마켓/입장/행사/투어 뒤에 바로 숫자, 영문 대문자, [, 제, 第 등이 오면 줄바꿈
  result = result.replace(/(행사|투어|체험|마켓|입장|페스타|축제)\s*([0-9\[제第A-Z])/g, '$1\n\n$2');
  
  // 12. 마켓/입장 뒤에 바로 지역명이 오면 줄바꿈 추가
  const locationNames = ['동명동', '충장로', '양림동', '대인동', '금남로', '송정역', '상무', '첨단', '수완', '유스퀘어', '전일빌딩', '광주', '대인'];
  for (const loc of locationNames) {
    const regex = new RegExp(`(마켓|입장|페스타|축제|투어)\\s*(${loc})`, 'g');
    result = result.replace(regex, '$1\n\n$2');
  }
  
  // 13. 이모지 뒤에 한글이 바로 오면 띄어쓰기 (이미 있으면 무시)
  result = result.replace(/([\u{1F300}-\u{1F9FF}])([가-힣])/gu, '$1 $2');
  
  // 14. 마지막 정리 - 연속된 줄바꿈 정리
  result = result.replace(/\n{4,}/g, '\n\n');
