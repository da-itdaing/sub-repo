...
const sanitizeMarkdown = (text) => {
  ...
  // 1. 한글키:값 형태가 "-"로 연결된 경우 줄바꿈으로 분리
  // 예: "E-5- 기간:" → "E-5\n- 기간:"
  // 예: "무료입장- 특징:" → "무료입장\n- 특징:"
  result = result.replace(/([가-힣0-9a-zA-Z,.:]+)-\s*([가-힣]+):/g, '$1\n- $2:');
  
  // 2. 줄 중간에 공백없이 붙은 "-한글:" 패턴 분리
  // 예: "내용입니다- 위치:" → "내용입니다\n- 위치:"
  result = result.replace(/([^\n\s-])-([가-힣]+):/g, '$1\n- $2:');
  
  // 3. 줄 시작의 "-한글" 패턴을 "- 한글" 형태로 정리 (공백 추가)
  result = result.replace(/^-([가-힣])/gm, '- $1');
  
  // 4. 붙어있는 리스트 항목 수정: "-영문" → "- 영문"
  result = result.replace(/^-([^\s])/gm, '- $1');
  
  // 5. 여러 개의 연속 빈 줄을 하나로 통합
  result = result.replace(/\n{3,}/g, '\n\n');
  
  // 6. 리스트 항목 앞의 불필요한 공백(1-3칸) 제거
  result = result.replace(/^\s{1,3}(-|\d+\.)/gm, '$1');
  
  // 7. 굵은 글씨 뒤에 바로 리스트가 오면 줄바꿈 추가
  result = result.replace(/\*\*([^*]+)\*\*\s*\n-/g, '**$1**\n\n-');
  
  // 8. "•", "·", "▪" 등의 리스트 마커를 "-" 리스트로 변환
  result = result.replace(/^[•·▪►▸]\s*/gm, '- ');
  
  // 9. 이모지 깨짐 수정 (불완전한 UTF-8)
  result = result.replace(/[\uFFFD]/g, '');
  
  // 10. 제목 뒤 바로 리스트가 오면 줄바꿈 추가
  // 예: "월곡동 문화축제- 위치:" → "월곡동 문화축제\n- 위치:"
  result = result.replace(/([가-힣]+축제|마켓|페스타|박람회|전시회)\s*-\s*([가-힣]+):/g, '$1\n- $2:');
