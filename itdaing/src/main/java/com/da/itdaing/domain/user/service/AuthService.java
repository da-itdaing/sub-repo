...
public class AuthService {
    ...
    public AuthDto.SignupResponse signupConsumer(AuthDto.SignupConsumerRequest request) {
        ...
        assignDefaultProfileImageIfEmpty(user);
    ...
    public AuthDto.SignupResponse signupSeller(AuthDto.SignupSellerRequest request) {
        ...
        // 판매자 프로필 생성 및 저장
        ImagePayload profileImage = resolveImagePayload(request.getProfileImage());
        ...
        SellerProfile profile = SellerProfile.builder()
                ...
                .profileImageUrl(profileImage != null ? profileImage.url() : null)
                .profileImageKey(profileImage != null ? profileImage.key() : null)
        ...
        AuthDto.SellerProfileInfo profileInfo = AuthDto.SellerProfileInfo.builder()
                ...
                .profileImage(com.da.itdaing.domain.file.dto.ImagePayload.builder()
                    ...
                    .key(profile.getProfileImageKey())
                    .build())
    ...
    public AuthDto.LoginResponse login(AuthDto.LoginRequest request) {
        ...
        Users user = userRepository.findByLoginId(request.getLoginId())
                .orElseThrow(() -> {
                    log.warn("Login attempt failed: unknown loginId={}", request.getLoginId());
                    return new AuthException(ErrorCode.INVALID_CREDENTIALS);
                });

        // 비밀번호 검증
        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            log.warn("Login attempt failed: invalid password for loginId={}", request.getLoginId());
    ...
    @Transactional
    public AuthDto.UserProfileResponse updateProfileImage(Long userId, ImagePayload payload) {
        Users user = userRepository.findById(userId)
            .orElseThrow(() -> new AuthException(ErrorCode.USER_NOT_FOUND));
        ImagePayload resolved = resolveImagePayload(payload);
        if (resolved == null) {
    ...
    private void assignDefaultProfileImageIfEmpty(Users user) {
        if (user.getProfileImageUrl() == null) {
            ...
            if (fallback != null) {
                user.updateProfileImage(fallback.url(), fallback.key());
            }
        }
    }

    private ImagePayload resolveImagePayload(ImagePayload payload) {
        if (payload != null && StringUtils.hasText(payload.url())) {
            return payload;
        }
